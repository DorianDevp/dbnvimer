...
M.groups = {
  global = {
    ...
    keys = {
      ...
      { lhs = "e", action = "export", desc = "export a table or the last result" },
  ...
  sidebar = {
    ...
    keys = {
      ...
      { lhs = "gE", action = "export", desc = "export this table" },
...
Only navigation and inspection are mapped, so nothing shadows an editing key.]],
    keys = {
      ...
      { lhs = "gx", action = "fixture", desc = "extract this row plus everything it needs" },
...
under the cursor, so a one-liner is three keystrokes from anywhere.]],
  ...
  export = {
    title = "Export editor",
    description = [[
Every export setting as text in a buffer: edit it, `:w` runs it. A preset sets
the settings that only make sense together — "for Excel" means a BOM *and*
CRLF *and* a semicolon when the locale writes `1,5`.]],
    keys = {
      { lhs = "gp", action = "preset", desc = "apply a preset" },
      { lhs = "gP", action = "preview", desc = "preview without writing anything" },
      { lhs = "gs", action = "save_preset", desc = "save these settings as a preset" },
      { lhs = "gr", action = "run", desc = "run the export" },
      { lhs = "q", action = "close", desc = "close without exporting" },
      { lhs = "g?", action = "help", desc = "show this help" },
    },
  },
...
dollar quoting and `DELIMITER`.]],
  ...
  result = {
    ...
    keys = {
      ...
      { lhs = "gt", action = "transpose", desc = "transposed view of this row" },
      { lhs = "gy", action = "yank", desc = "yank cell, row or selection as..." },
      { lhs = "ge", action = "export", desc = "export: formats, encodings, partitioning" },
